...
import { updateData } from "../../services/admin";
...
const ModifyData = () => {
  ...
  const [inputData, setInputData] = useState({
    id:state?.data?.[0]?.id,
    heading: state?.data?.[0]?.heading,
    image: null,
    video: null,
    giff: null,
    products: [
      {
        title: state?.data?.[0]?.cards?.[0]?.title,
        description: state?.data?.[0]?.cards?.[0]?.description,
      },
      {
        title: state?.data?.[0]?.cards?.[1]?.title,
        description: state?.data?.[0]?.cards?.[1]?.description,
      },
      {
        title: state?.data?.[0]?.cards?.[2]?.title,
        description: state?.data?.[0]?.cards?.[2]?.description,
      },
    ],
  });
        ...
        {inputData.products.map((product, index) => (
          <div key={index}>
            ...
            <input
              ...
              type="text"
              name={`title${index + 1}`}
              value={inputData?.products?.[index]?.title}
              placeholder="Title"
              className="mt-[0.3rem] border border-[#000] rounded-[5px] focus:outline-[#ff9f1c] block text-[1.2rem] w-[100%] p-[0.4rem]"
            />
            <input
              ...
              value={inputData?.products?.[index]?.description}
              className="mt-[0.3rem] border border-[#000] rounded-[5px] focus:outline-[#ff9f1c] block text-[1.2rem] w-[100%] p-[0.4rem]"
            />
          </div>
        ))}
